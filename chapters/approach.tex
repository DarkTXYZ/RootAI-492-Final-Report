...
\begin{enumerate}
  \item \textbf{\texttt{reward-function}}: The MCTS must determine how ``good'' the state at the end of a rollout is. This is done using the reward-function. There are 4 options for this parameters:
  \begin{itemize}
    \item \texttt{win}: considers whether the player wins at the current state; 0
    \item \texttt{vp-difference}:
    \item \texttt{vp-difference-bin}:
    \item \texttt{vp-difference-relu}:
  \end{itemize}  
\end{enumerate}
