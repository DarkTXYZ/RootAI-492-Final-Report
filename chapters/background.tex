...
\section{Game tree}
A \Gls{game-tree} is a directed graph that represents possible game states and decisions within a game. Each vertex represents a state of the game and each out going edge represents a decision at that state(vertex) which will lead the another state(vertex). The size of the game tree can be used to measure the complexity a game as it represents all the possible way a game can pan out. 
