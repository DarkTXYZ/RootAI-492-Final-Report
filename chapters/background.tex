...
\cite{RN2020} One of the most efficient tree policies used in MCTS is UCB (Upper Confidence Bound). As UCB is used in bandit problems, the process in selection phase can be modelled as a multi-armed bandit problem. A child node j is selected to maximize:

\[ \textit{UCB} = \overline{X_j} + 2C_p\sqrt{\frac{2\ln n}{n_j}} \]

when $n$ is the number of times a parent node has visited, $n_j$ is the number of times a child node has visited, $X_j$ is the average reward of the parent node, and $C_p > 0$ is a constant.

In the formula, the two terms are the exploitation term and the exploration term, respectively. If a node has been explored a few times, the exploration term will be high as a denominator $n_j$. $C_p$ is a constant that balances between exploitation and exploration.

\subsection{Winning action selection} \label{winning-action}
MCTS continues to run four steps until the endgame condition is met or the computation budget is reached. Then, the action of the root node is selected by some criteria. \newline \cite{mcts-survey} Browne says that there are four criteria for selecting winning action, based on their survey:
...
\cite{mcts-survey} Also, we handle the uncertainty by using \textit{determinization}, the process of sampling all possible outcomes of all chance events, making it fully observable.
